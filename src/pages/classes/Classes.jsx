...
import { Users, BookOpen, MoreHorizontal, FilePlus2, Search, Edit2, Trash2 } from 'lucide-react';
...
export default function Classes() {
  ...
  const load = () => Promise.all([getClasses(), getSubjects()]).then(([c, s]) => { setData(c); setSubjects(s); setLoading(false); }).catch(() => setLoading(false));
  useEffect(() => { load(); }, []);
  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          <Button onClick={() => setModalOpen(true)} icon={FilePlus2}>Create Class</Button>
        }
      />
      ...
      {filtered.length === 0 && !loading && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-16 text-center">
          ...
          <p className="text-sm text-gray-500">Try adjusting your search or create a new class.</p>
        </div>
      )}
