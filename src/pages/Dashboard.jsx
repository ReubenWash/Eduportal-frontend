...
function SchoolAdminDashboard({ stats, loading, loadError }) {
    ...
    const fetchSubmissionStatus = async () => {
      ...
      try {
        const data = await getSubmissionStatus({
          termId: stats.activeTerm.id
        });
        setSubmissionStatus(Array.isArray(data) ? data : []);
      } catch (err) {
  ...
  const submissionRows = submissionStatus.slice(0, 5).map(s => ({
    teacher: s.teacher?.firstName && s.teacher?.lastName 
      ...
      : s.teacher || 'Not Assigned',
    subject: s.subject?.name || s.subject || 'Unknown Subject',
    class: s.className || s.class?.name || '—',
    done: s.submitted || false,
  ...
  const statsCards = [
    ...
    { title: 'Active Staff', value: stats.totalStaff != null ? new Intl.NumberFormat().format(stats.totalStaff) : '—', icon: Users, color: 'blue' },
...
function SubjectTeacherDashboard({ user, stats }) {
  ...
  const statCards = [
    { title: 'My Subjects', value: stats?.mySubjects || '0', icon: BookOpen, color: 'indigo' },
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="bg-gradient-to-br from-indigo-600 to-indigo-700 px-5 py-4">
              ...
              <p className="text-white font-bold text-3xl">{assignments.length > 0 ? `${submitted}/${assignments.length}` : '—'}</p>
            ...
            <div className="px-5 py-4">
              ...
              <p className="text-xs text-gray-500">
                {pending > 0
                  ...
                  : assignments.length > 0 ? 'All scores submitted — great work!' : 'No assignments yet'}
              </p>
            </div>
          </div>
